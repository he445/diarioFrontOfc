...
export const api = {
  ...
  update: async (post: Post) => {
    const postUpdated = await axios.patch(`/post/${post.id}`, post);
    return postUpdated;
  },
  delete: async (postId: string) => {
    const isDeleted = await axios.delete(`post/${postId}`);
    return isDeleted;
  },
  creatUser: async (users: User) => {
    console.log(localStorage);
    try {
      const newUser = await axios.post("/users", users);
      return newUser;
    } catch (err) {
      console.log(err);
    }
  ...
  updateUser: async (user: newUser, userId: string) => {
    const userUpdated = await axios.patch(`/users/${userId}`, user);
    return userUpdated;
  },

  deleteUser: async (userId: string) => {
    const isDeleted = await axios.delete(`users/${userId}`);
    return isDeleted;
  },
};
